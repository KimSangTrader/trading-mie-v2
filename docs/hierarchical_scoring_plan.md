...
#### Phase 6-3 최초 라이브 실행 결과 및 안전 문제 발견 [2026-08-27]
- 사용자가 EC2에서 `systemctl start mie-v2-trade-execution.service`로 최초 수동
  실행. 결과: KIS 토큰 발급 성공, 보유/후보 종목 일봉 조회 대부분 성공(1종목
  462860만 HTTP 500 - 신규상장 등으로 데이터 부족 추정, `build_market_data_and_candidates()`가
  `len(closes) < atr_period+1` 조건으로 이미 안전하게 건너뛰도록 설계돼 있어
  파이프라인 자체는 안 죽음), 청산/피라미딩 대상 없음(보유 포지션이 아직 없어서
  당연함), 신규진입 7종목 검토했으나 0종목 진입.
- **🔴 발견 1(안전 문제) - 주문 실행이 실전(PROD) 서버로 연결되고 있었음**:
  토큰 발급 URL이 `openapi.koreainvestment.com:9443`(PROD_BASE_URL)였다 - EC2
  `.env`의 `ENVIRONMENT=production`을 main.py(시세 수집용으로 의도적으로 그렇게
  설정된 것 - 시세 조회는 계정 종류와 무관하게 안전)뿐 아니라 이 파이프라인의
  `KISClient()`도 그대로 물려받고 있었다 - 사용자가 확정한 "모의투자부터 시작"과
  반대. 이번엔 아래 발견 2 덕분에 실제 주문까지는 안 갔지만, 그 우연이 없었다면
  **실전 계좌로 진짜 매수 주문이 나갈 뻔한 상황**이었다. 즉시 조치: `data/kis_client.py`
  `KISClient.__init__`에 `environment` 파라미터를 추가하고, 이 파일의 `__main__`이
  `.env`의 ENVIRONMENT 대신 별도의 `MIE_TRADE_ENVIRONMENT`(기본값 "development"
  =모의투자, 명시적으로 "production"을 추가해야만 실전 전환)로 KISClient를 만들도록
  수정 - 두 파일 변경이력의 【2026-08-27】항목 참고. 기존 `KISClient()`(인자 없이
  호출하는 main.py 등)는 동작 변화 없음(하위 호환).
- **발견 2(설정 오류, 사용자 조치 필요) - 계좌번호 형식**: `잔고조회 오류:
  계좌번호를 CANO(8자리)/ACNT_PRDT_CD(2자리)로 분리할 수 없습니다 (길이=8)`.
  `.env`의 `KIS_PROD_ACCOUNT_NUMBER`(모의투자 전환 후에는 `KIS_DEV_ACCOUNT_NUMBER`도
  동일하게 확인 필요)가 계좌상품코드(보통 "01") 없이 8자리 CANO만 들어있는 것으로
  보인다 - `"12345678-01"` 또는 `"1234567801"`(대시 없이 10자리) 형식으로 수정
  필요. `_split_account()`가 이 오류를 추측 없이 즉시 예외로 잡아낸 덕분에 잘못된
  계좌 조합으로 조용히 진행되지 않고 바로 드러난 것 - 설계가 의도대로 작동했다.
  **이 오류의 연쇄 효과**: `get_balance()` 실패 → `run_entry_pipeline()`의
  `available_cash`가 0.0으로 처리 → 계산상 모든 종목의 `target_shares`가 0이 됨
  → 신규진입 7종목 전부가 실제 필터링이 아니라 이 오류 때문에 걸러진 것("entered":
  0은 정상 동작이 아니라 이 버그의 결과). 계좌번호를 고치고 재실행하면 실제
  진입 여부가 달라질 수 있음.
- **다음 라이브 실행 전 사용자 확인 필요**: (1) `.env`에
  `MIE_TRADE_ENVIRONMENT=development`가 되어 있는지(또는 아예 없어서 기본값
  "development"가 적용되는지) 재확인, (2) `KIS_DEV_ACCOUNT_NUMBER`/
  `KIS_PROD_ACCOUNT_NUMBER` 형식(8-2, 대시 포함/미포함 모두 허용) 수정,
