...
【2026-08-26, Phase 6-2】check_stop()은 손절가 도달 여부만 보는 단순 버전으로
남겨둔다(하위 호환용) - 실제 파이프라인 배선 시에는 exit_engine.analyze_exit()을
써야 한다. 그때는 miev2tradingsell.txt 문서가 없어서 "전량 청산 규칙은 다음
단계에서 확인 후 추가"라고 미뤄뒀었는데, 그 문서가 도착해서 exit_engine.py에
긴급청산/분석논리붕괴/부분익절/트레일링/터틀청산까지 포함한 상위 호환 버전을
구현했다. should_add()는 여전히 이 파일이 유일한 구현이다(추가매수 판정은
새 문서의 범위 밖).
